...
Versioning code is not a new concept and has been used for decades[^1]. It has lead to version control systems such as git. However, in this post we are interested in versioing to communicate development. 
...
Although not discussed in this post there are alternatives to  semantic versioning, such as calendar versioning[^2]. Some of the critique raised for semantic versioning is it's lack of objective criteria for version incrementing, a breaking change or a small bug fix can be easily identified and correspond to major and patch version increments, but other changes are more uncertain to which version number they correspond to. Additionally semantic versioning implying only breaking changes warrant major releases, but a vast re-write of a code base may also justify a major version change. Different breaking changes have different magnitudes, therefore a change to a single exported function or a change to every exported function will be communicated in a single, equal, version increment. 
